...
def generate_bricks(rows, cols):
    ...
    for row in range(rows):
        for col in range(cols):
            brick = Brick(col * brick_width + gap * col, row * brick_height + gap * row, brick_width, brick_height, 2, [GREEN, RED])
            bricks.append(brick)
...
def main():
    ...
    paddle = Paddle(paddle_x, paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, LIGHT_GRAY)
    ball = Ball(WIDTH/2, paddle_y - BALL_RADIUS, BALL_RADIUS, LIGHT_GRAY)
    ...
    def display_text(text):
        text_render = LIVES_FONT.render(text, 1, WHITE)
